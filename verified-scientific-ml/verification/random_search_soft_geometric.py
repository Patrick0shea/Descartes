"""
Random testing baseline over the same domain D and
SOFT_GEOMETRIC_MOMENTUM_PROPERTIES as
verification/verify_soft_geometric.py, for direct comparison against
Marabou's formal verification results on Model C. Reuses
...
to the baselines used for Model A and Model B.
...
from verification.export_soft_geometric_onnx import load_soft_geometric_surrogate
...
RESULTS_PATH = os.path.join(ARTIFACTS_DIR, "soft_geometric_random_testing_results.json")
...
def main() -> None:
    ...
    start = time.time()
    results = run_random_test(model, SOFT_GEOMETRIC_MOMENTUM_PROPERTIES, N_SAMPLES, SEED)
