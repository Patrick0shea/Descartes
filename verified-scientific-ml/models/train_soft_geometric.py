"""
Train the soft-geometric surrogate (Model C) on the same two-particle
spring system dataset as models/train_particle.py (Model A) and
models/train_geometric.py (Model B), for direct three-way comparison.

Learns the same one-step map, with the same train/val/test split, same
optimizer/loss/seed/epoch budget -- the only difference from the other
training scripts is the architecture (models/particle_soft_geometric.py)
and that NO input/output normalization is applied here. Normalization is
unnecessary; the relative position/velocity features the network actually
...
Training configuration (fixed, for reproducibility):
    ...
    architecture      = ParticleSoftGeometricMLP: delta_net 4 -> 16 -> 16 -> 4,
                        ReLU activations, translation-invariant by
                        construction (see models/particle_soft_geometric.py)
...
def main() -> None:
    ...
    torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, "particle_soft_geometric.pt"))
    np.savez(
    ...
    print(f"Saved model to {os.path.join(CHECKPOINT_DIR, 'particle_soft_geometric.pt')}")
