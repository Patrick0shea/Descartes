"""
Evaluate the trained soft-geometric surrogate (Model C): test-set
metrics, momentum-conservation analysis, loss curve, and multi-step
rollout vs. the true simulator -- the exact same analysis run for
Model A (models/evaluate_particle.py) and Model B
(models/evaluate_geometric.py), so the three are directly comparable.

Run from the verified-scientific-ml/ directory, after
models/train_soft_geometric.py:
...
HIDDEN_DIM = 16  # must match models/train_soft_geometric.py

# Same rollout initial condition as models/evaluate_particle.py and
# models/evaluate_geometric.py, for a like-for-like comparison.
...
def load_artifacts():
    ...
    model.load_state_dict(
        torch.load(os.path.join(CHECKPOINT_DIR, "particle_soft_geometric.pt"))
    )
    model.eval()

    loss_history = np.load(
        os.path.join(CHECKPOINT_DIR, "particle_soft_geometric_loss_history.npz")
    )
...
def predict(model: ParticleSoftGeometricMLP, x: np.ndarray) -> np.ndarray:
...
def plot_loss_curve(train_losses: np.ndarray, val_losses: np.ndarray) -> None:
    ...
    ax.set(xlabel="epoch", ylabel="MSE (physical units)", yscale="log",
           title="Soft-geometric surrogate: training and validation loss")
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS_DIR, "soft_geometric_loss_curve.png"), dpi=150)
    plt.close(fig)
...
def plot_momentum_error_hist(delta_px: np.ndarray, delta_py: np.ndarray) -> None:
    ...
    axes[0].set(xlabel="delta_Px (predicted)", ylabel="count",
                title="Soft-geometric: momentum error x")
    axes[1].hist(delta_py, bins=60, color="tab:red", alpha=0.8)
    axes[1].set(xlabel="delta_Py (predicted)", ylabel="count",
                title="Soft-geometric: momentum error y")
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS_DIR, "soft_geometric_momentum_error_hist.png"), dpi=150)
    plt.close(fig)
...
def plot_rollout_trajectories(true_states: np.ndarray, nn_states: np.ndarray) -> None:
    ...
    ax.plot(nn_states[:, 0], nn_states[:, 1], "--", label="particle 1 (soft-geometric NN)")
    ax.plot(nn_states[:, 4], nn_states[:, 5], "--", label="particle 2 (soft-geometric NN)")
    ax.set(xlabel="x", ylabel="y",
           title="Rollout: particle paths, soft-geometric NN vs true simulator")
    ax.legend()
    ax.set_aspect("equal")
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS_DIR, "soft_geometric_rollout_trajectories.png"), dpi=150)
    plt.close(fig)
...
def plot_momentum_drift(t, px_true, py_true, px_nn, py_nn) -> None:
    ...
    axes[0].plot(t, px_nn, "--", label="soft-geometric NN rollout")
    axes[0].set(xlabel="time", ylabel="Px",
                title="Total momentum (x): soft-geometric NN vs true")
    axes[0].legend()

    axes[1].plot(t, py_true, label="true (simulator)")
    axes[1].plot(t, py_nn, "--", label="soft-geometric NN rollout")
    axes[1].set(xlabel="time", ylabel="Py",
                title="Total momentum (y): soft-geometric NN vs true")
    axes[1].legend()

    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS_DIR, "soft_geometric_momentum_drift.png"), dpi=150)
    plt.close(fig)
...
def main() -> None:
    ...
    for tol, pct in momentum["pct_within_tol"].items():
        lines.append(
            f"  tol={tol:.0e}:  Px within: {pct['px']:.2f}%   Py within: {pct['py']:.2f}%   magnitude within: {pct['magnitude']:.2f}%"
        )
    ...
    lines += [
        ...
        f"Final |momentum drift| soft-geometric NN rollout: dPx={rollout_final_drift[0]:.6e}, dPy={rollout_final_drift[1]:.6e}",
        f"Max |momentum drift| over rollout, soft-geometric NN: dPx={rollout_max_drift[0]:.6e}, dPy={rollout_max_drift[1]:.6e}",
    ...
    with open(
        os.path.join(CHECKPOINT_DIR, "particle_soft_geometric_evaluation_report.txt"), "w"
    ) as f:
        f.write(report + "\n")
